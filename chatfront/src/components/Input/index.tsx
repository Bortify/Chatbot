...
export default function Input({ socket, util, isServerIdle }: { socket: Socket, util: any, isServerIdle:boolean }) {
  ...
  function sendMessage() {
    if (inputRef.current && inputRef.current.value.length > 0) {
      ...
      const newChat = { message: inputRef.current.value, author: 'CLIENT' }
      const chatArr = [...chat, newChat]
      setChat(chatArr)
